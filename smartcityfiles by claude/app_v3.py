...
.hero { 
  background: linear-gradient(135deg, rgba(0, 217, 255, 0.08) 0%, rgba(13, 17, 23, 0.9) 50%, rgba(46, 204, 113, 0.05) 100%);
...
.hero-sub { font-size: 14px; color: #8b949e; line-height: 1.5; margin: 0; }
.live-badge { 
  display: inline-flex; align-items: center; gap: 6px;
  color: #00d9ff; font-weight: 700; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase;
  background: rgba(0, 217, 255, 0.15); padding: 4px 12px; border-radius: 20px; 
  border: 1px solid rgba(0, 217, 255, 0.3); 
}
.live-dot { width: 6px; height: 6px; background: #00d9ff; border-radius: 50%; box-shadow: 0 0 8px #00d9ff; animation: blink 1.6s infinite; }
@keyframes blink { 0%,100%{opacity:1} 50%{opacity:.2} }

.city-panel { 
  background: linear-gradient(145deg, #21262d 0%, rgba(33, 38, 45, 0.8) 100%);
  border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 14px; 
  padding: 20px 24px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2); 
}
.city-panel-title { 
...
.mc { 
  background: linear-gradient(145deg, #21262d 0%, rgba(33, 38, 45, 0.8) 100%);
  border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 14px; 
  padding: 18px 20px; overflow: hidden; position: relative; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2);
  transition: all 0.3s ease; 
}
.mc:hover { border-color: rgba(255, 255, 255, 0.15); box-shadow: 0 4px 16px rgba(0, 0, 0, 0.3); transform: translateY(-2px); }
.mc::after { content: ''; position: absolute; top: 0; left: 0; width: 4px; height: 100%; border-radius: 4px 0 0 4px; }
.mc.blue::after { background: #00d9ff; box-shadow: 0 0 12px rgba(0, 217, 255, 0.4); } 
.mc.teal::after, .mc.green::after { background: #2ecc71; box-shadow: 0 0 12px rgba(46, 204, 113, 0.4); }
.mc.amber::after { background: #f59e0b; box-shadow: 0 0 12px rgba(245, 158, 11, 0.4); } 
.mc.red::after { background: #ef4444; box-shadow: 0 0 12px rgba(239, 68, 68, 0.4); }
.mc.purple::after { background: #a78bfa; box-shadow: 0 0 12px rgba(167, 139, 250, 0.4); }
.mc.sky::after { background: #00d9ff; box-shadow: 0 0 12px rgba(0, 217, 255, 0.4); }

.mc-lbl { 
  font-family: 'Inter', sans-serif; font-size: 12px; font-weight: 600; text-transform: uppercase;
  letter-spacing: 0.05em; color: #8b949e; margin-bottom: 8px; 
}
.mc-val { 
  font-family: 'Inter', monospace; font-size: 32px; font-weight: 700;
  color: #ffffff; line-height: 1; margin-bottom: 4px; 
}
.mc-unit { font-size: 14px; color: #8b949e; font-weight: 500; margin-left: 4px; }
.mc-sub { font-size: 11px; color: #8b949e; margin-top: 6px; }

.sh { 
  display: flex; align-items: center; gap: 14px;
  border-bottom: 1px solid rgba(255, 255, 255, 0.08); padding-bottom: 12px; margin-bottom: 16px; 
}
.sh-icon { 
  width: 42px; height: 42px; border-radius: 12px;
  display: flex; align-items: center; justify-content: center; font-size: 20px; 
  background: rgba(0, 217, 255, 0.15) !important; color: #00d9ff; box-shadow: 0 0 20px rgba(0, 217, 255, 0.15); 
}
.sh-title { font-family: 'Inter', sans-serif; font-size: 16px; font-weight: 600; color: #ffffff; letter-spacing: -0.01em; margin: 0; }
.sh-desc { font-size: 12px; color: #8b949e; margin: 2px 0 0 0; }

.db { 
  display: block; padding: 14px 20px; border-radius: 10px;
  font-family: 'Inter', sans-serif; font-size: 18px; font-weight: 700;
  letter-spacing: 0.05em; text-align: center; width: 100%; border: 1px solid transparent; 
}
.db-HIGH   { background: rgba(239, 68, 68, 0.15); border-color: rgba(239, 68, 68, 0.4); color: #ef4444; }
.db-MEDIUM { background: rgba(245, 158, 11, 0.15); border-color: rgba(245, 158, 11, 0.4); color: #f59e0b; }
.db-LOW    { background: rgba(46, 204, 113, 0.15); border-color: rgba(46, 204, 113, 0.4); color: #2ecc71; }

.ib{ background: rgba(0, 217, 255, 0.05); border-left: 3px solid #00d9ff; border-radius: 0 8px 8px 0; padding: 12px 16px; font-size: 13px; color: #00d9ff; margin: 8px 0; }
.wb{ background: rgba(245, 158, 11, 0.05); border-left: 3px solid #f59e0b; border-radius: 0 8px 8px 0; padding: 12px 16px; font-size: 13px; color: #f59e0b; margin: 8px 0; }
.sb{ background: rgba(46, 204, 113, 0.05); border-left: 3px solid #2ecc71; border-radius: 0 8px 8px 0; padding: 12px 16px; font-size: 13px; color: #2ecc71; margin: 8px 0; }
.eb{ background: rgba(239, 68, 68, 0.05); border-left: 3px solid #ef4444; border-radius: 0 8px 8px 0; padding: 12px 16px; font-size: 13px; color: #ef4444; margin: 8px 0; }

.sug{ background: rgba(33, 38, 45, 0.6); border-left: 3px solid #a78bfa; border-radius: 0 8px 8px 0; padding: 10px 14px; margin: 6px 0; font-size: 13px; color: #e0e7ff; }

.bw{ 
  background: linear-gradient(145deg, #21262d 0%, rgba(33, 38, 45, 0.8) 100%);
  border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; padding: 14px; text-align: center; 
  transition: all 0.3s; 
}
.bw:hover { transform: translateY(-2px); border-color: rgba(255, 255, 255, 0.15); box-shadow: 0 4px 16px rgba(0, 0, 0, 0.3); }
.bid{ font-size: 10px; color: #8b949e; text-transform: uppercase; margin-bottom: 6px; font-weight: 600; letter-spacing: 0.05em; }
.bbo{ width: 30px; height: 60px; background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 4px; margin: 0 auto 8px; position: relative; overflow: hidden; }
.bfi{ position: absolute; bottom: 0; left: 0; right: 0; border-radius: 2px; transition: height 0.8s ease; }
.bp{ display: inline-flex; align-items: center; justify-content: center; padding: 3px 8px; border-radius: 6px; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
.bp-FULL    { background: rgba(239, 68, 68, 0.15); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.4); }
.bp-CRITICAL{ background: rgba(239, 68, 68, 0.3); color: #ef4444; border: 1px solid #ef4444; box-shadow: 0 0 10px rgba(239, 68, 68, 0.4); }
.bp-HALF    { background: rgba(245, 158, 11, 0.15); color: #f59e0b; border: 1px solid rgba(245, 158, 11, 0.4); }
.bp-QUARTER { background: rgba(46, 204, 113, 0.15); color: #2ecc71; border: 1px solid rgba(46, 204, 113, 0.4); }
.bp-EMPTY   { background: rgba(46, 204, 113, 0.05); color: #2ecc71; border: 1px solid rgba(46, 204, 113, 0.2); }

.fb{ margin: 6px 0; }
.fbl{ font-size: 11px; color: #8b949e; margin-bottom: 4px; }
.fbt{ height: 6px; background: rgba(255, 255, 255, 0.08); border-radius: 3px; overflow: hidden; position: relative; }
.fbf{ height: 100%; border-radius: 3px; transition: width 0.8s ease; }

.cb{ background: rgba(46, 204, 113, 0.07); border: 1px solid rgba(46, 204, 113, 0.2); border-radius: 8px; padding: 10px 14px; font-size: 13px; color: #2ecc71; margin: 8px 0; font-weight: 500; }

.stTabs [data-baseweb="tab-list"]{ gap: 16px; background: transparent; border-bottom: 1px solid rgba(255, 255, 255, 0.08); padding-bottom: 8px; }
.stTabs [data-baseweb="tab"]{ background: transparent; border: none; border-bottom: 2px solid transparent; color: #8b949e; font-size: 14px; font-weight: 500; padding: 8px 0; border-radius: 0; }
.stTabs [aria-selected="true"]{ background: transparent !important; color: #00d9ff !important; border-bottom-color: #00d9ff !important; box-shadow: none !important; }

div[data-baseweb="select"]>div,
div[data-baseweb="input"] input{
  background: #21262d !important; border: 1px solid rgba(255, 255, 255, 0.08) !important;
  border-radius: 8px !important; color: #f0f6fc !important; padding: 10px 12px; }
.stButton>button {
  background: linear-gradient(135deg, #00d9ff 0%, rgba(0, 217, 255, 0.8) 100%);
  border: none; border-radius: 10px; color: #000; font-weight: 600; font-size: 14px; padding: 11px 22px; width: 100%;
  box-shadow: 0 2px 12px rgba(0, 217, 255, 0.4); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
}
.stButton>button:hover{ transform: translateY(-2px); box-shadow: 0 6px 24px rgba(0, 217, 255, 0.4); color: #000; }
.stSlider div[data-baseweb="slider"]>div>div { background: #00d9ff !important; }
.stSlider div[role="slider"] { background: #fff !important; border: 2px solid #00d9ff !important; box-shadow: 0 0 10px rgba(0, 217, 255, 0.5) !important; }
